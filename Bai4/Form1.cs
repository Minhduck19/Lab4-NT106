...
namespace Bai4
{
    public partial class Form1 : Form
    {
        ...
        private void ShowBooking(Movie m)
        {
            Form f = new Form { Text = "Đặt vé: " + m.Title, Size = new Size(400, 300), StartPosition = FormStartPosition.CenterParent };

            Label l1 = new Label { Text = "Tên Khách:", Top = 20, Left = 20 };
            TextBox t1 = new TextBox { Top = 20, Left = 100, Width = 200 };

            Label l2 = new Label { Text = "Số lượng:", Top = 60, Left = 20 };
            NumericUpDown num = new NumericUpDown { Top = 60, Left = 100, Minimum = 1, Value = 1 };

            Label lTotal = new Label { Text = "Tổng tiền: 45,000 đ", Top = 100, Left = 100, ForeColor = Color.Red, Font = new Font("Arial", 12, FontStyle.Bold), AutoSize = true };

            num.ValueChanged += (s, ev) => lTotal.Text = $"Tổng tiền: {(num.Value * 45000):N0} đ";

            Button bOk = new Button { Text = "Thanh Toán", Top = 150, Left = 100, Width = 100, BackColor = Color.Green, ForeColor = Color.White, DialogResult = DialogResult.OK };

            f.Controls.AddRange(new Control[] { l1, t1, l2, num, lTotal, bOk });

            if (f.ShowDialog() == DialogResult.OK)
            {
                MessageBox.Show($"Đặt vé thành công!\nPhim: {m.Title}\nKhách: {t1.Text}\nSố lượng: {num.Value}\nTổng tiền: {(num.Value * 45000):N0} đ");
            }
        }
    }
}